<footer>
    <div class="container">
        <div class="ftr__first">
            <div class="ftr__logo">
                <img src="<?php echo $base_url; ?>assets/imgs/logo/ftr_logo.svg" alt="">
            </div>
            <div class="ftr_srvlist">
                <h4>TopPrep</h4>
                <ul>
                    <li><a href="<?php echo $base_url;?>top-prep/sat-prep.php">SAT</a></li>
                    <li><a href="<?php echo $base_url;?>top-prep/gre.php">GRE</a></li>
                    <li><a href="<?php echo $base_url;?>top-prep/gmat.php">GMAT</a></li>
                    <li><a href="<?php echo $base_url;?>top-prep/ap.php">AP</a></li>
                    <li><a href="<?php echo $base_url;?>top-prep/act-prep.php">ACT</a></li>
                    <li><a href="#">IELTS</a></li>
                    <li><a href="#">TOEFL</a></li>
                </ul>
            </div>
            <div class="ftr_srvlist">
                <h4>Student Service</h4>
                <ul>
                    <li><a href="<?php echo $base_url;?>study-abroad/overview.php">Study Abroad</a></li>
                    <li><a href="<?php echo $base_url;?>study-abroad/admissions-consulting-and-profile-building.php">Admissions Consulting<br> and Profile Building</a></li>
                    <li><a href="<?php echo $base_url;?>study-abroad/early-profile-building-and-mentoring.php">Early Profile Building<br> and Mentoring</a></li>
                    <li><a href="<?php echo $base_url;?>study-abroad/early-profile-building-and-mentoring/comprehensive-plan-with-training.php">Comprehensive Plan<br> with Training</a></li>
                </ul>
            </div>
            <div class="ftr_srvlist">
                <h4>Important Links</h4>
                <ul>
                    <li><a href="<?php echo $base_url;?>about-us.php">About Us</a></li>
                    <!-- <li><a href="#">Success Stories</a></li>
                    <li><a href="#">FAQs</a></li>
                    <li><a href="#">Blogs</a></li>
                    <li><a href="#">News and Events</a></li> -->
                    <li><a href="<?php echo $base_url;?>contact.php">Contact Us</a></li>
                </ul>
            </div>
        </div>
        <div class="ftr_address">
            <ul class="adrsl">
                <li>
                    <h4>Address</h4>
                    <p>401/402/403, Dheeraj Plaza, Above Colors Showroom, Opp. Bandra Police Station, Hill Road, Bandra (W), Mumbai 400050, India.</p>
                </li>
                <li>
                    <h4>Call us</h4>
                    <a href="tel:+918108800760">+91 81088 00760</a>
                </li>
                <li>
                    <h4>Get Social</h4>
                    <ul class="socm">
                        <li><a href="https://www.linkedin.com/company/eduedge-global/" target="_blank" alt="Linkedin" width="20" height="19"><img src="<?php echo $base_url;?>assets/imgs/logo/linkedin.svg" loading="lazy" alt="LinkedIn"></a></li>
                        <li><a href="https://www.facebook.com/eduedge.global/" target="_blank"><img src="<?php echo $base_url; ?>assets/imgs/logo/fb.svg" loading="lazy" alt="Facebook" width="20" height="20"></a></li>
                         <li><a href="https://www.instagram.com/eduedge.global?igsh=MTlnZjdiNDBic3B5cA==" target="_blank"><img src="<?php echo $base_url; ?>assets/imgs/logo/insta.svg" alt="Instagram" loading="lazy" width="20" height="20"></a></li>
                    </ul>
                </li>
            </ul>
        </div>
        <!-- <div class="ftr-tabs" data-tab-section>
            <ul class="tab-buttons">
                <li class="tb-cntnt active" data-tab="tab1"><span>SAT</span></li>
                <li class="tb-cntnt" data-tab="tab2"><span>GRE</span></li>
                <li class="tb-cntnt" data-tab="tab3"><span>GMAT</span></li>
                <li class="tb-cntnt" data-tab="tab4"><span>PSAT</span></li>
                <li class="tb-cntnt" data-tab="tab5"><span>AP</span></li>
                <li class="tb-cntnt" data-tab="tab6"><span>ACT</span></li>
                <li class="tb-cntnt" data-tab="tab7"><span>IELTS</span></li>
                <li class="tb-cntnt" data-tab="tab8"><span>TOEFL</span></li>
            </ul>
            <div class="tabs-content">
                <div class="tbs active" id="tab1">
                    <ul class="tbs-flx">
                        <li><a href="#">SAT Coaching in India</a></li>
                        <li><a href="#">Best SAT Prep Courses</a></li>
                        <li><a href="#">Personalized SAT Preparation</a></li>
                        <li><a href="#">SAT Classes Near Me</a></li>
                        <li><a href="#">SAT Crash Course Online</a></li>
                        <li><a href="#">SAT 1-on-1 Tutoring</a></li>
                        <li><a href="#">Kaplan SAT Test Prep India</a></li>
                        <li><a href="#">SAT Classes in Mumbai</a></li>
                        <li><a href="#">SAT Preparation for Indian Students</a></li>
                        <li><a href="#">Online SAT Practice Tests</a></li>
                        <li><a href="#">Expert SAT Mentoring</a></li>
                        <li><a href="#">High Score SAT Strategy</a></li>
                        <li><a href="#">SAT Prep with Ivy League Mentors</a></li>
                        <li><a href="#">Affordable SAT Training</a></li>
                    </ul>
                </div>
                <div class="tbs" id="tab2"></div>
                <div class="tbs" id="tab3"></div>
                <div class="tbs" id="tab4"></div>
                <div class="tbs" id="tab5"></div>
                <div class="tbs" id="tab6"></div>
                <div class="tbs" id="tab7"></div>
                <div class="tbs" id="tab8"></div>
            </div>
        </div> -->
        <div class="ftr__second">
            <ul class="flgs_list">
                <li>
                    <a>
                        <img src="<?php echo $base_url; ?>assets/imgs/logo/usa.svg" loading="lazy" alt="USA Flag" width="48" height="48">
                        <h4>Study in USA</h4>
                    </a>
                </li>
                <li>
                    <a>
                        <img src="<?php echo $base_url; ?>assets/imgs/logo/ire.svg" loading="lazy" alt="Ireland Flag" width="48" height="48">
                        <h4>Study in Ireland</h4>
                    </a>
                </li>
                <li>
                    <a>
                        <img src="<?php echo $base_url; ?>assets/imgs/logo/aus.svg" loading="lazy" alt="Australia Flag" width="48" height="48">
                        <h4>Study in Australia</h4>
                    </a>
                </li>
                <li>
                    <a>
                        <img src="<?php echo $base_url; ?>assets/imgs/logo/nz.svg" loading="lazy" alt="New Zealand Flag" width="48" height="48">
                        <h4>Study in New Zealand</h4>
                    </a>
                </li>
                <li>
                    <a>
                        <img src="<?php echo $base_url; ?>assets/imgs/logo/can.svg" loading="lazy" alt="Canada Flag" width="48" height="48">
                        <h4>Study in Canada</h4>
                    </a>
                </li>
                <li>
                    <a>
                        <img src="<?php echo $base_url; ?>assets/imgs/logo/uk.svg" loading="lazy" alt="UK Flag" width="48" height="48">
                        <h4>Study in UK</h4>
                    </a>
                </li>
                <li>
                    <a>
                        <img src="<?php echo $base_url; ?>assets/imgs/logo/china.svg" loading="lazy" alt="Ireland Flag" width="48" height="48">
                        <h4>Study in China</h4>
                    </a>
                </li>
                <li>
                    <a>
                        <img src="<?php echo $base_url; ?>assets/imgs/logo/kh.svg" loading="lazy" alt="Australia Flag" width="48" height="48">
                        <h4>Study in Hongkong</h4>
                    </a>
                </li>
                <li>
                    <a>
                        <img src="<?php echo $base_url; ?>assets/imgs/logo/jp.svg" loading="lazy" alt="New Zealand Flag" width="48" height="48">
                        <h4>Study in Japan</h4>
                    </a>
                </li>
                <li>
                    <a>
                        <img src="<?php echo $base_url; ?>assets/imgs/logo/eu.svg" loading="lazy" alt="Canada Flag" width="48" height="48">
                        <h4>Study in EU</h4>
                    </a>
                </li>
            </ul>
        </div>
        <div class="ftr__third">
            <div class="rgtrv">
                <a>EduEdgePro © 2025, Inc. All Rights Reserved.</a>
            </div>
            <div class="policies">
                <a href="<?php echo $base_url;?>privacy-policy.php">Privacy Policy</a>
            </div>
        </div>
    </div>
</footer>

<script>
    document.addEventListener('DOMContentLoaded', function () {
    const ftrAccrd = document.querySelectorAll('.ftr_srvlist');

    ftrAccrd.forEach(function (ftrItem) {
        ftrItem.addEventListener('click', function () {
            // Close all others
            ftrAccrd.forEach(function (el) {
                if (el !== ftrItem) {
                    el.classList.remove('active');
                }
            });

            // Toggle the clicked one
            ftrItem.classList.toggle('active');
        });
    });
});

</script>